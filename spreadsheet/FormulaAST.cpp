...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        auto operand = operand_->Evaluate(sheet);
        if (type_ == UnaryMinus) {
            return -(operand);
        } 
        return operand;
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        ...
        const auto* cell_ptr = sheet.GetCell(*cell_);

        if (!cell_ptr) {
            return 0.0;
        }

        auto cell_val = cell_ptr->GetValue();

        if (std::holds_alternative<FormulaError>(cell_val)) {
            throw std::get<FormulaError>(cell_val);
        } else if (std::holds_alternative<double>(cell_val)) {
            return std::get<double>(cell_val);
        } else if (std::holds_alternative<std::string>(cell_val)) {
            std::string str_val = std::get<std::string>(cell_val);
            
            if (!str_val.empty()) {
                try {
                    std::size_t num_digit = 0;
                    int result = std::stoi(str_val, &num_digit);
                    
                    if (num_digit < str_val.size()) {
